...
import Slot8TrialResult from './Slot8TrialResult';
...
export default class Slot8Controller extends cc.Component {
    ...
    @property
    colPadding: number = 8; // 5

    @property
    iconHeight: number = 112;  // 112
    ...
    chooseLineScript: Slot8ChooseLine = null;
    @property(cc.Node)
    ...
    settingMusic() {
        ...
        cc.sys.localStorage.setItem("music_Slot_8", "" + this.musicSlotState);
    }

    start() {
        // musicSave :   0 == OFF , 1 == ON
        console.log("start slot 8");
        var musicSave = cc.sys.localStorage.getItem("music_Slot_8");
        if (musicSave != null) {
            this.musicSlotState = parseInt(musicSave);
        } else {
            this.musicSlotState = 1;
            cc.sys.localStorage.setItem("music_Slot_8", "1");
        }
    ...
    randomFirst() {
        for (let i = 0; i < this.listCol.length; i++) {
            for (let j = 0; j < this.numberOfItemInCol; j++) {
                ...
                icon.getComponent("Slot8Icon").setSprite(this.listItemSpirte[randomSprite]);
                // if (randomSprite < 3) {
                //     icon.getComponent("Slot8Icon").setSpine(randomSprite);
                // } else {
                //     icon.getComponent("Slot8Icon").setSprite(this.listItemSpirte[randomSprite]);
                // }
            }
    ...
    spinClick() {
        ...
        if (!this.isTrial) {
            ...
            var rIdx = Utils.randomRangeInt(0, Slot8TrialResult.results.length);
            this.spinResult(Slot8TrialResult.results[rIdx]);
        }
    }
    ...
    spinEffect(matrix: Array<number>) {
        ...
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 5; j++) {
                ...
                item.getComponent("Slot8Icon").setSprite(this.listItemSpirte[sprite]);
                // if (sprite < 3) {
        ...
        for (let i = 0; i < this.listCol.length; i++) {
            ...
            let newPos = cc.v2(col.position.x, -(this.numberOfItemInCol - 3) * (this.colPadding + this.iconHeight));
    ...
    spinResult(res: cmd.ReceiveResult | any) {
        ...
        if (successResult.indexOf(result) === -1) {
            ...
            this.toggleFast.interactable = true;

            this.setEnableAllButton(true);
            if (result === 102) {
    ...
    actSpin(){
        this.btnSpin.interactable = !this.btnSpin.interactable;
        if(!this.btnSpin.interactable){
            this.btnSpin.node.color = cc.Color.GRAY;
            this.setButtonAuto(false);
            this.setButtonFlash(false);
            this.spinClick();
        }else{
            this.btnSpin.node.color = cc.Color.WHITE;
        }
    }
                            ...
                            cc.sequence(
                                ...
                                cc.callFunc(() => {
                                    ...
                                    for (let i = 0; i < mapLineArr.length; i++) {
                                        ...
                                        node.getComponent("Slot8Icon").scale();
                                    }
                                }),
    ...
    setEnableAllButton(active: boolean){
        this.setButtonEnable(active);
    ...
    changeSpeed() {
        ...
        if (this.toggleFast.isChecked && this.toggleTrial.isChecked) {
            this.toggleFast.isChecked = false;
            this.showToast("Tính năng này không hoạt động ở chế độ chơi thử.");
            return;
        }
        this.SPEED = this.toggleFast.isChecked ? 0.5 : 1;
        //this.setButtonFlash(!this.toggleFast.isChecked);
        if (this.toggleFast.isChecked) {
            this.spinClick();
            this.setButtonAuto(false);
            this.setButtonEnable(false);
        }else{
            this.setButtonAuto(true);
    ...
    toggleAutoOnCheck() {
        ...
        this.setButtonAuto(!this.toggleAuto.isChecked);
