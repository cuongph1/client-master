...
export default class LotoController extends cc.Component {
    ...
    actionSubmitBet() {
        cc.log("LOTO actionSubmitBet GAME_LOCATION : ", this.GAME_LOCATION);
        cc.log("LOTO actionSubmitBet GAME_MODE : ", this.GAME_MODE);
        cc.log("LOTO actionSubmitBet GAME_CHANNEL : ", this.GAME_CHANNEL);
        cc.log("LOTO actionSubmitBet channelsOpen : ", this.channelsOpen);
        cc.log("LOTO actionSubmitBet modesOpen : ", this.modesOpen);

        if (this.channelsOpen.indexOf(this.GAME_CHANNEL) == -1) {
            // Khong co -> Khong duoc choi -> show popup
            let msg_1 = "Đài " + cmd.Code.LOTO_CHANNEL_NAME[this.GAME_CHANNEL] + " hiện đang đóng";
            this.showPopupNotify(msg_1);
            return;
        }

        if (this.modesOpen.indexOf(this.GAME_MODE) == -1) {
            // Khong co -> Khong duoc choi -> show popup
            let msg_2 = cmd.Code.LOTO_GAME_MODE_NAME[this.GAME_MODE] + " hiện đang đóng";
            this.showPopupNotify(msg_2);
            return;
        }
    ...
    requestGetLotoResult(sessionId, channelId) {
        ...
        ShootFishNetworkClient.getInstance().request("LOTO5", {
            ...
            if (resData["channel"] == 0 || resData["session"] == 0) {
                ...
                if (result.length == 8) { // k co giai 8
                    if (this.popupResult.active) {
                        // Popup Result
