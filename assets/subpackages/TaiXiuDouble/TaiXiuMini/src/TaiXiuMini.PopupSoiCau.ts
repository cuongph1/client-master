                ...
                for (var i = 0; i < count; i++) {
                    ...
                    if (_isTai !== isTai) {
                        if (curData.length > maxItem) {
                            var quotient = Math.floor(curData.length/maxItem);
                            var remainder =curData.length%maxItem;
                            for (var k = 0; k < quotient; k++) {
                              var cdata= curData.splice(0, maxItem);
                              data.push(cdata);
                            }
                            var xdata= curData.splice(0, remainder );
                            data.push(xdata);
                        }
                        else
                        {
