...
export class TabMomo {
    ...
    start() {
        ...
        //     node.parent = this.itemFactorTemplate.parent;
        //     node.active = true;
        //     let sale = Configs.App.SERVER_CONFIG.ratioNapMomo - 1;
        //     let saleString = sale > 0 ? Math.ceil(sale * 100) + "%" : "0%";
        //     let menhGia = Configs.App.SERVER_CONFIG.listMenhGiaNapThe[i];
        //     let nhan = Math.ceil(menhGia * Configs.App.SERVER_CONFIG.ratioNapMomo);
        //     node.getChildByName("menhgia").getComponent(cc.Label).string = Utils.formatNumber(menhGia);
        //     node.getChildByName("khuyenmai").getComponent(cc.Label).string = saleString;
        //     node.getChildByName("nhan").getComponent(cc.Label).string = Utils.formatNumber(nhan);
        // }
        //get config from server 
        App.instance.showLoading(true);
        this.lblTransNote.string = "Nội dung chuyển tiền: nap "+ Configs.Login.Nickname;
        Http.get(Configs.App.API, { "c": 130 }, (err, res) => {
            App.instance.showLoading(false);
            if (err == null) {
                if(res.momoConfig === undefined){

                    return;
                }
                this.lblAccountName.string = res.momoConfig.accountName;
                this.lblPhone.string = res.momoConfig.accountNumber;
...
export class TabBank {
    ...
    start() {
        this.lblTransNote.string = "Nội dung chuyển khoản: nap "+Configs.Login.Nickname;
        App.instance.showLoading(true);
        Http.get(Configs.App.API, { "c": 130 }, (err, res) => {
            App.instance.showLoading(false);
            if (err == null) {
